	...
	},
	{
		...
		Desc: i18n.String{
			i18n.EN: here.Doc(`
				...
				A value already set is kept. Otherwise set to ~TZ~ when the invoice's
				ordering section names an ~issuer~, and to ~CC~ when the invoice is
				~self-billed~ and the parties carry identifiers proving they differ.
				Left unset when a self-billed invoice's supplier and customer are the
				same party, as in an autofattura for self-consumption (TD27), where
				the supplier is the one filing the document.
			`),
		},
		Values: []*cbc.Definition{
			{
